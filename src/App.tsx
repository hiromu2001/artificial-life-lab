...
function 世界を描く(canvas: HTMLCanvasElement, 世界: シミュレーション, 選択id: number | null) {
  ...
  ctx.strokeStyle = 'rgba(117, 255, 174, 0.055)'
  ctx.lineWidth = 1
  ...
  for (const 生物 of 世界.個体一覧) {
    ...
    if (選択中) {
      ...
      ctx.strokeStyle = 'rgba(117, 255, 174, 0.22)'
      ctx.lineWidth = 1
      ctx.stroke()
    }
    ctx.save()
...
function App() {
  ...
  useEffect(() => {
    ...
    const ループ = () => {
      ...
      if (世界 && 世界Canvas.current && 脳Canvas.current && 推移Canvas.current) {
        const 選択個体 = 選択Ref.current === null ? null : 世界.個体一覧.find((o) => o.id === 選択Ref.current) ?? null
  ...
  const 世界 = 世界Ref.current
  const 選択個体 = useMemo(() => {
    if (!世界 || 選択id === null) return null
    return 世界.個体一覧.find((o) => o.id === 選択id) ?? null
  }, [世界, 選択id, 世界?.時刻])

  const canvasクリック = (e: React.MouseEvent<HTMLCanvasElement>) => {
  ...
  const 結果保存 = () => {
    ...
    const 内容 = JSON.stringify({
      ...
      時刻: 世界.時刻,
      統計: 世界.統計,
      生存個体: 世界.個体一覧.map((o) => ({ id: o.id, 世代: o.世代, 年齢: o.年齢, エネルギー: o.エネルギー, 子供数: o.子供数, 食べた餌数: o.食べた餌数, 速度: o.遺伝子.速度, 視野: o.遺伝子.視野 })),
    }, null, 2)
    ...
    <main className="app-shell">
      ...
      <section className="toolbar card">
        <div className="button-row">
          ...
          <button onClick={() => 新しい実験()}>同じ条件でやり直す</button>
          <button onClick={おすすめ設定}>おすすめで新規実験</button>
        </div>
      ...
      <section className="main-grid">
        <div className="card world-card">
          <div className="section-heading">
            <div><h2>生態系</h2><p>生物をクリックすると、その個体の状態と脳が見られます。</p></div>
      ...
      <section className="card settings-card">
        <div className="section-heading">
          <div><h2>実験条件</h2><p>数字を変えて「この条件でやり直す」を押すと、環境の違いを比較できます。</p></div>
        ...
        <div className="button-row settings-actions"><button className="primary" onClick={() => 新しい実験()}>この条件でやり直す</button><button onClick={結果保存}>実験結果を保存</button></div>
      </section>

      <section className="explanation card">
        <h2>何が起きているの？</h2>
        <p>各生物は「餌の位置」「他の生物の位置」「自分のエネルギー」など10個の情報を受け取ります。その情報を小さなニューラルネットワークに通して、前進・左右への旋回・食べる、の4つから1つを選びます。</p>
        <p>餌をうまく食べてエネルギーが増えた生物は子供を作ります。子供の脳は親に似ていますが、少しだけ突然変異します。つまり、人間が正解を教えなくても、何世代も経つうちに生き残りやすい行動が増えるかを観察できます。</p>
      </section>
    </main>
  )
}
